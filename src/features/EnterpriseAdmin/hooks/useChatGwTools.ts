 ...
 *
 * chat-gw 的 `tools/list` 响应已按 Casdoor 角色过滤过,heichat 这侧不再做
 * 任何额外过滤。
 */
...
export interface ChatGwToolMeta {
  category: string;
  description?: string;
  /** heichat builtin identifier, e.g. `chatgw-cloud_cost-dashboard_overview` */
  identifier: string;
...
/** chat-gw 工具名(带 `.`)→ heichat builtin identifier(只允许 [a-zA-Z0-9_-])*/
export function chatGwNameToIdentifier(name: string): string {
  return `chatgw-${name.replaceAll('.', '-')}`;
}

/** 反向:heichat identifier → chat-gw 原名(只替换第一个 `-`,因为分类段可能有下划线)*/
...
/** 把 chat-gw 工具转成 heichat ToolManifest,每个工具只有一个 `execute` api */
