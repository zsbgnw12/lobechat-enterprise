...
| `SKILLHUB_URL`                                          | 可选。自托管 SkillHub 根地址。未配时不出现 SkillHub 页和导入入口           |
| `SKILLHUB_TOKEN`                                        | 可选。SkillHub / ClawHub Bearer token，读私有命名空间时需要                |
...
| `SKILLHUB_URL`                    | 可选。SkillHub 注册中心根 URL，明文注入 `SKILLHUB_URL`。未配置时适配器休眠，界面不展示 SkillHub 页和导入入口。详见 [`skillhub.md`](skillhub.md)                                                                 |
| `SKILLHUB_TOKEN`                  | 可选。有值时以 `secretref:skillhub-token` 注入。SkillHub 私有 / 内部技能需要。等级：高                                                                                                                          |
