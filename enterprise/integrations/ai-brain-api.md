...
| Casdoor Bearer Token | `Authorization: Bearer <token>`  | 内部系统间调用（client\_credentials） | token roles 非空时用 token；为空时 fallback 到 DB `users.roles` |
...
┌────────────────────────────────────────────┬─────────────┬───────────────┬───────────────┬───────────────┐
│ 前端页面 │ cloud\_admin │ cloud\_ops │ cloud\_finance │ cloud\_viewer │
├────────────────────────────────────────────┼─────────────┼───────────────┼───────────────┼───────────────┤
│ 仪表盘 / (Dashboard) │ ✅ │ ✅ │ ✅ │ ✅ │
├────────────────────────────────────────────┼─────────────┼───────────────┼───────────────┼───────────────┤
...
- API Key 管理
- Azure 部署和 OAuth 授权
- 数据范围：看到全量数据，不受 cloud\_account\_grant 限制
...
- **机器应用**（client\_credentials）：Casdoor token 不携带角色，管理员通过 SQL 设置 DB 中的 `users.roles`
...
### 2.1 获取 Token（Casdoor client\_credentials）
...
| `by_provider[]`            | array   | 按 provider 当月累计，`cost` + `percentage`（占 total\_cost 百分比，保留两位小数） |
...
| `record_count`  | integer               | 命中的 billing\_data 行数                                           |
...
[
  {
    "date": "2026-04-01",
    "usage_quantity": "2327753993.353517",
    "cost": "7399.860517",
    "record_count": 2745
  },
  {
    "date": "2026-04-02",
    "usage_quantity": "1300687882.204744",
    "cost": "6175.577245",
    "record_count": 2974
  }
]
...
[
  {
    "product": "Vertex AI",
    "usage_quantity": "55518458336.183727",
    "usage_unit": "month",
    "cost": "124410.727567",
    "record_count": 58924
  },
  {
    ...
    "usage_quantity": "18315835.000000",
    "usage_unit": "count",
    "cost": "315.170942",
    "record_count": 101
  }
...
[
  {
    "category_id": 2,
    "name": "default",
    "original_cost": 24846.617897,
    "markup_rate": 1.15,
    "final_cost": 28573.61058155
  }
]
```
...
```json
[{ "project_id": "xianlong-2", "name": "xianlong-2", "provider": "gcp", "cost": 58749.779907 }]
```
...
| `provider`                           | string           | `aws`/`gcp`/`azure`（来自 supply\_source）                       |
...
```json
{
  ...
  "history": [
    ...
    {
      "id": 36,
      "action": "customer_unbound",
      "from_status": "active",
      "to_status": "active",
      "operator": "sales",
      "customer_code": "SYNC_C1",
...
| `secret_fields`                       | string\[]      | 账号凭据字段**名**（不含值）；例如 aws\_access\_key\_id、client\_secret |
...
```json
{
  "daily": [{ "date": "2026-04-01", "cost": 45.23, "usage_quantity": 12.0 }],
  "daily_by_service": [
    {
      "date": "2026-04-01",
  ...
  "services": [
    {
      ...
      "cost": 5280.532382,
      "usage_quantity": 2690.11845,
      "usage_unit": "Units"
    },
    {
      "service": "Claude Sonnet 4.6 (Amazon Bedrock Edition)",
      "cost": 1246.2185079999997,
      "usage_quantity": 1582.6575039999998,
      "usage_unit": "Units"
    }
  ],
...
| `services[].service`         | 产品名（对应 billing\_data.product）             |
...
| 字段             | 说明                                                |
| ---------------- | --------------------------------------------------- |
| `data_source_id` | FK → data\_sources（对应 /api/data-sources/ 条目）  |
| `category_id`    | FK → categories（费用分类 markup\_rate）；可为 null |
...
| 客户编号分配（销售专用） | `POST /api/service-accounts/customer-assignments/sync`、`PUT /api/service-accounts/{id}` 带 `customer_codes` | 仅销售系统走 Casdoor client\_credentials 下发；AI 不应写 |
